...
import type { PartialFile } from '@/services/sendProtocol'
    ...
    <ul v-if="filesReceived.length > 0" class="flex flex-col gap-2">
      ...
      >
        <div class="flex-1 flex gap-4 flex-row">
          <div class="flex flex-1 flex-col">
            ...
            <p class="text-sm text-slate-400">
              <span class="font-mono"
                >{{ file.type.length == 0 ? 'Unknown Type' : file.type }}
              </span>
              -
          ...
          <a
            class="bg-gray-800 flex items-center justify-center cursor-pointer p-2 rounded hover:bg-gray-700"
            :href="getUrl(file)"
            download
          >
            <svg
    ...
    >
      <div class="flex-1 flex gap-4 flex-row">
        <div class="flex flex-1 flex-col">
          ...
          <p class="text-sm text-slate-400">
            <span class="font-mono"
              >{{ currentFile.type.length == 0 ? 'Unknown Type' : currentFile.type }}
            </span>
            -
          ...
          <div class="h-2 bg-gray-700 rounded-full overflow-hidden mt-2">
            <div
              :style="{ width: `${Math.round(currentFile.progress() * 100)}%` }"
              class="h-full bg-emerald-600"
              :class="currentFile.progress() < 1 && 'animate-pulse'"
            ></div>
          </div>
        </div>
      </div>
